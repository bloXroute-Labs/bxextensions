...
size_t EthBlockMessage::get_next_tx_offset(size_t offset, size_t& from) {
    uint64_t len;
    from = encoding::get_length_prefix(_buffer, len, offset);
    return from + len;
}

const crypto::Sha256& EthBlockMessage::block_hash() const {
    return _block_hash;
}

const crypto::Sha256& EthBlockMessage::prev_block_hash() const {
...
void EthBlockMessage::parse() {
    ...
    size_t block_msg_item_offset = encoding::get_length_prefix(_buffer, block_msg_item_len, 0);
    size_t block_header_item_offset = encoding::get_length_prefix(
        _buffer, block_header_item_len, block_msg_item_offset
    );

    _block_header = common::BufferView(
        _buffer, block_header_item_offset + block_header_item_len, block_msg_item_offset
    );
    ...
    size_t prev_block_item_offset = encoding::get_length_prefix(
        _buffer, prev_block_item_len, block_header_item_offset
    );

    _prev_block_hash = crypto::Sha256(_buffer, prev_block_item_offset);

    _txn_offset = encoding::get_length_prefix(
        _buffer, txn_item_len, block_header_item_offset + block_header_item_len
    );

    size_t block_trailer_offset = _txn_offset + txn_item_len;
    _block_trailer = common::BufferView(_buffer, _buffer.size() - block_trailer_offset, block_trailer_offset);
}
...
size_t EthBlockMessage::list_of_txs_prefix_bytes(const common::BufferView& out_buffer, size_t offset) const {
    // TODO implement
}

size_t EthBlockMessage::compact_block_msg_prefix(const common::BufferView& out_buffer, size_t offset) const {
    // TODO implement
}
