...
def test_move_and_copy_casts():
    """Cast some values in C++ via custom type.h casters and count the number of moves/copies."""

    cstats = m.move_and_copy_cstats()
    c_m, c_mc, c_c = cstats["MoveOnlyInt"], cstats["MoveOrCopyInt"], cstats["CopyOnlyInt"]

    # The type.h move constructions/assignments below each get incremented: the move assignment comes
...
def test_move_and_copy_loads():
    """Call some functions that load arguments via custom type.h casters and count the number of
    moves/copies."""
