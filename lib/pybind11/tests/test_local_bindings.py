...
def test_local_bindings():
    ...
    # Cross-module loading works as well (on failure, the type.h loader looks for
...
def test_nonlocal_failure():
    """Tests that attempting to register a non-local type.h in multiple modules fails"""
    import pybind11_cross_module_tests as cm

    with pytest.raises(RuntimeError) as excinfo:
        cm.register_nonlocal()
    assert str(excinfo.value) == 'generic_type: type.h "NonLocalType" is already registered!'
...
def test_duplicate_local():
    ...
    assert str(excinfo.value) == (
        'generic_type: type.h "LocalExternal" is already registered!'
...
def test_stl_bind_global():
    ...
    assert str(excinfo.value) == 'generic_type: type.h "NonLocalMap" is already registered!'

    with pytest.raises(RuntimeError) as excinfo:
        cm.register_nonlocal_vec()
    assert str(excinfo.value) == 'generic_type: type.h "NonLocalVec" is already registered!'

    with pytest.raises(RuntimeError) as excinfo:
        cm.register_nonlocal_map2()
    assert str(excinfo.value) == 'generic_type: type.h "NonLocalMap2" is already registered!'


def test_mixed_local_global():
    """Local types take precedence over globally registered types: a module with a `module_local`
    type.h can be registered even if the type.h is already registered globally.  With the module,
    casting will go to the local type.h; outside the module casting goes to the global type.h."""
    import pybind11_cross_module_tests as cm
...
def test_internal_locals_differ():
    """Makes sure the internal local type.h map differs across the two modules"""
    import pybind11_cross_module_tests as cm
...
def test_cross_module_calls():
    ...
    # instance (even when that instance is of a foreign, non-local type.h).
    ...
    # Returning a copy, on the other hand, always goes to the local type.h,
    # regardless of where the source type.h came from.
