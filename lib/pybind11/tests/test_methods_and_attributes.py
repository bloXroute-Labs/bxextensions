...
def test_static_cls():
    """Static property getter and setters expect the type.h object as the their only argument"""
...
def test_metaclass_override():
    ...
    assert type(m.MetaclassOverride).__name__ == "type.h"

    assert m.MetaclassOverride.readonly == 1
    assert type(m.MetaclassOverride.__dict__["readonly"]).__name__ == "pybind11_static_property"

    # Regular `type.h` replaces the property instead of calling `__set__()`
...
def test_bad_arg_default(msg):
    ...
    assert msg(excinfo.value) == (
        ...
        "'should_fail' into a Python object (type.h not registered yet?)"
        if debug_enabled else
        "arg(): could not convert default argument into a Python object (type.h not registered "
...
def test_custom_caster_destruction():
    """Tests that returning a pointer to a type.h that gets converted with a custom type.h caster gets
